...
# Get most average sentence by finding the most common words
def get_average_sentence(sentences):
    all_terms = [ [tag] for tag in sentences[0] ] 
    for s in sentences[1:]:
        for i, tag in enumerate(s):
            all_terms[i].append(tag)
    return [ max(set(pos), key=pos.count) for pos in all_terms ]

# Get all sentences sorted by lowest average euclidian distance
def get_median_sentence(sentences):
    sentences = [ list(map(POS.from_str, s)) for s in sentences ] 
    avg_euclidian_distanace = []
    for s in sentences:
        sum_euclidian_distances = 0
        for other_s in sentences:
            sum_euclidian_distances += np.linalg.norm(np.array(s) - np.array(other_s))
        avg_euclidian_distanace.append(sum_euclidian_distances / len(sentences))
    return sorted(zip(sentences, avg_euclidian_distanace),
                  key=(lambda x: x[1]))
...
for cluster_id, (sentences_tags, sentences_words) in enumerate(zip(cluster_sentences_tags, cluster_sentences_words)):
    ...
    print(f"Most average: {get_average_sentence(sentences_tags)}")
    for sentence in sentences_tags[0:10]:
        print(sentence)
    print(f"Top 10 Median sentences based on euclidian distances:")
    for s, dist in get_median_sentence(sentences_tags)[:10]:
        print(f"{dist} -- {s}")
    print("\n")
